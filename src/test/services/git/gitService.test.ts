...
import { GitService, getGitService, WorkspaceProvider } from '../../../services/git/gitService';
...
suite('GitService Test Suite', () => {
   ...
   */
  function createMockWorkspaceProvider(dir: string | undefined): WorkspaceProvider {
    return {
  ...
  suite('stageFiles', () => {
    test('should stage specified files', async () => {
      ...
      assert.ok(status.staged.includes('file1.ts'), 'file1.ts should be staged');
      assert.ok(!status.staged.includes('file2.ts'), 'file2.ts should not be staged');
    });
  ...
  suite('unstageFiles', () => {
    test('should unstage specified files', async () => {
      ...
      const status = await git.status();
      assert.ok(!status.staged.includes('unstage1.ts'), 'unstage1.ts should be unstaged');
      assert.ok(status.staged.includes('unstage2.ts'), 'unstage2.ts should remain staged');
    });
  });
  ...
  suite('stageAndCommit', () => {
    test('should stage specific files and commit them', async () => {
      ...
      const status = await git.status();
      assert.ok(status.not_added.includes('sc2.ts'), 'sc2.ts should still be untracked');
    });
  });
